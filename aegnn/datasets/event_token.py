...
class E2SRC_Module(nn.Module):
    ...
    def __init__(self, shape, group_num, patch_size):
        ...
        self.pixels_per_patch = self.patch_size[0]*self.patch_size[1]
        self.b = 1e-4

    def get_token(self, x):
        PH, PW = int((self.H + 1) / self.patch_size[0]), int((self.W + 1) / self.patch_size[1])
        N = x.shape[0]
        #Adapt the order for ncars dataset
        ...
        col_patch = torch.div(ex, (self.W / PW + self.b), rounding_mode='floor').long()
        row_patch = torch.div(ey, (self.H / PH + self.b), rounding_mode='floor').long()
        token     = (torch.floor(ex % (self.W / PW + self.b)) +
                    torch.floor(ey % (self.H / PH + self.b)) * int((self.W + 1) / PW)).long()

        t_min  = et[0]; t_max = et[-1]
        dtime  = torch.floor(self.time_div * (et - t_min) / (t_max - t_min + 1e-6)).long()
        dtime  = dtime.clamp(0, self.time_div - 1)
        pol    = ep.long()

        patch_features = y[0, :, row_patch, col_patch].T  # [K*2*2*patch_size[0]*patch_size[1], N]

        patch_features_avg = patch_features.reshape(N, -1, self.pixels_per_patch).mean(dim=2)

        return patch_features_avg
